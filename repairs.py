...
def main():
    ...
    if view == "New Update":
        ...
        assessor_appointed = json.dumps(pd.to_datetime(st.date_input("Date Assessor Appointed")))
        ...
        date_authorized = json.dumps(pd.to_datetime(st.date_input("Date Repair Authorized")))

        repair_amount = st.text_input("Repair Amount") 
    
        release_date = json.dumps(pd.to_datetime(st.date_input("Release Date")))
